...
positions = list(map(int, input('введите позиции для поиска произведения через пробел: ').split()))
composition = 1
for i in positions:
    composition*=elements[i-1]
print(f'Произведение элементов списка {elements}, находящихся на позициях {positions} = {composition}')